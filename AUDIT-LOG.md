...
> system were extracted out of this repo, so the files, findings, and test suite
> referenced below that concern either one — `src/lib/email/`,
> `netlify/functions/`, `SubscribeForm`, the journey pages,
> `tests/email-flow.test.mjs`, `src/lib/pins.mjs`, `PinSaveGallery`, the Pin
> preview pages, `scripts/export-pins.mjs` — no longer exist here. Everything
> else — the entry schema and publish gate, the CSV scripts, the URL builders,
> the daily resolver — is unchanged and still accurate.
